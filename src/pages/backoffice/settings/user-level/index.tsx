...
import {iUserLevel,iPagin} from 'lib/interface';
...
interface iUserLevelPage {
    datum: any;
}


const UserLevelPage: React.FC<iUserLevelPage> = (datum) =>{
    const [datumUserLevel,setDatumUserLevel]= useState<Array<iUserLevel>>([]);
    ...
    useEffect(() => {
         setDatumUserLevel(datum.datum.data);
    ...
    const handleLoadData = async (val: string) => {
        let url = Api.apiClient+`user_level`;
        if(val!==null){
            url+=`?${val}`;
        }
        await handleGet(url,(datum)=>{
            setDatumUserLevel(datum.data);
            setArrData(datum);
        })
        ...
        <Layout title="UserLevel">
            <div className="container mt-6 px-2 lg:px-7 mx-auto grid mb-20">
                ...
                <br/>
                        ...
                        <tbody>
                            {
                            datumUserLevel?.length>0?datumUserLevel.map((item:iUserLevel,i:number)=>{
                                    return(
...
export async function getServerSideProps(ctx:NextPageContext) {
    ...
    //   const getData = await Api.get(Api.apiUrl +`user_level?page=1&datefrom=${moment(new Date()).format('YYYY-MM-DD')}&dateto=${moment(new Date()).format('YYYY-MM-DD')}`);
let datum=[];
  try {
      const getData = await Api.get(Api.apiUrl +`user_level?page=1&datefrom=${moment(new Date()).format('YYYY-MM-DD')}&dateto=${moment(new Date()).format('YYYY-MM-DD')}`);
